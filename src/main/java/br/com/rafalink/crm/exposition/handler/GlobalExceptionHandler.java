...
@RestControllerAdvice(basePackages = "br.com.rafalink.crm.exposition.controller.json")
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErroResponse> handleBusiness(BusinessException ex) {
        return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY)
                .body(new ErroResponse(422, "Regra de negócio violada", ex.getMessage(), LocalDateTime.now()));
    }

    @ExceptionHandler(InvalidStatusException.class)
    public ResponseEntity<ErroResponse> handleInvalidStatus(InvalidStatusException ex) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(new ErroResponse(400, "Status inválido", ex.getMessage(), LocalDateTime.now()));
    }
    ...
    public ResponseEntity<ErroResponse> handleValidation(MethodArgumentNotValidException ex) {
        ...
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(new ErroResponse(400, "Erro de validação", msg, LocalDateTime.now()));
    }
